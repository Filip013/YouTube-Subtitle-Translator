/**
 * 30-Second Classical Captions Playback Simulation
 * Feeds test_sample_30s_16k.wav to gemini-3.5-live-translate-preview
 * and logs second-by-second on-screen classical captions (0s - 30s).
 */
...
async function run30sSimulation() {
  console.log(`========================================================================`);
  console.log(`🎬 30-SECOND CLASSICAL CAPTIONS SIMULATION (Video 9OnmNLYvv5Y)`);
  ...
  return new Promise((resolve) => {
    ...
    const finalize = () => {
      ...
      if (trimmed) {
        ...
        const endMs = Math.max(startMs + 1200, lastAudioMs);
        renderer.addCue({
          text: trimmed,
          startMs: startMs,
          endMs: endMs
    ...
    ws.onmessage = async (event) => {
      ...
      try {
        ...
        if (data.setupComplete) {
          console.log(`📡 Connected! Streaming 30 seconds of YouTube audio...`);
          ...
          streamInterval = setInterval(() => {
            if (offset + chunkSize > pcm.length) {
              ...
              console.log(`\n📡 Finished sending all 30 seconds of audio frames.`);
    ...
    setTimeout(() => {
      ...
      console.log(`\n========================================================================`);
      console.log(`📺 FULL 30-SECOND PLAYBACK TIMELINE (Second-by-second):`);
      console.log(`========================================================================`);
